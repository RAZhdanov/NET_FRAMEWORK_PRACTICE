...
namespace ComplexApplication
{
    public class CComplex
    {
        ...
        //Также необходимо реализовать функцию создающую комплексное число по модулю и аргументу (например, static метод создания)
        public static CComplex GetComplexValueThroughModulusAndArgument(CComplex val)
        {
            double Re = (Math.Cos(val.Im) * val.Re);
            double Im = (Math.Sin(val.Im) * val.Re);
            return new CComplex(Re, Im);
        }


        public static double GetModule(double _Re, double _Im)
        {
            return Math.Sqrt(Math.Pow(_Re, 2) + Math.Pow(_Im, 2));
        }
        public static double GetParameter(double _Re, double _Im)
        {
